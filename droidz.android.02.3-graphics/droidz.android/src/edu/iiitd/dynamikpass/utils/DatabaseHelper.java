...
public class DatabaseHelper extends SQLiteOpenHelper{
     ...
     * Adding a droid
     */
    public long createDroid(Image image) {
        SQLiteDatabase db = this.getWritableDatabase();
     
        ContentValues values = new ContentValues();
        values.put(KEY_X, image.getX());
        values.put(KEY_Y, image.getY());
        values.put(KEY_BITMAP_ID, image.getBitmapId());
        values.put(KEY_COLOR, image.getColor());
    ...
    public List<String> getAllDroids() {
        ...
        if (c.moveToFirst()) {
            do {
                Image image = new Image(null, 0, 0, 0, selectQuery, null);
